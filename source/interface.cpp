...
void CLI::show_result_file_exist_on_folder(const vector<string>& file) {
    clear();
    cout << "Available File:\n";
    for (size_t i = 0; i < file.size(); i++) cout << i + 1 << ". " << file[i] << endl;
}
...
CLI::Plot_Graph_State CLI::plot_graph(const string& type) {
    ...
    if (type != "scalability") {
        ...
        if (file.size() >= 3) system("python graph/graph.py scalability result/");
        else show_failed_interface("PLOT GRAPH", "Need at least 2 datasets to plot scalability!");
    }
    waitting_screen();
...
CLI::Plot_Graph_State CLI::activity_graph() {
    while (true) {
        ...
        if (choice < 1 || choice > 5) { show_failed_interface("PLOT GRAPH", "Invalid Input!"); continue; }
        switch (choice) {
        case 1: return Plot_Graph_State::COST_GRAPH;
        case 2: return Plot_Graph_State::RUNTIME_GRAPH;
        case 3: return Plot_Graph_State::SCALABILITY_GRAPH;
        case 4: return Plot_Graph_State::QUALITY_VS_OPTIMAL;
        case 5: return Plot_Graph_State::EXIT;
        }
    }
}
...
CLI::State CLI::control_plot_graph() {
    ...
    while (pgs != Plot_Graph_State::EXIT) {
        switch (pgs) {
        case(Plot_Graph_State::GRAPH_MENU): pgs = activity_graph(); break;
        case(Plot_Graph_State::COST_GRAPH): pgs = plot_graph("cost"); break;
        case(Plot_Graph_State::RUNTIME_GRAPH): pgs = plot_graph("runtime"); break;
        case(Plot_Graph_State::SCALABILITY_GRAPH): pgs = plot_graph("scalability"); break;
        case(Plot_Graph_State::QUALITY_VS_OPTIMAL): pgs = plot_graph("quality"); break;
        case(Plot_Graph_State::EXIT): break;
        }
    }
    ...
    for (size_t i = 0; i < s.patch_selected.size(); i++) {
        if (s.patch_selected[i] == 1) {
            ...
            for (size_t j = 0; j < patches[i].size(); j++) {
                cout << patches[i][j];
                if (j + 1 < patches[i].size()) cout << ",";
            }
            cout << "]";
            if (count < s.num_patches) cout << ", ";
            count++;
        }
    }
    cout << endl;
...
CLI::Algorithm_Type CLI::activity_algo() {
    while (true) {
        show_algo_menu();
        int choice; cin >> choice; cin.ignore();
        if (choice < 1 || choice > 6) { show_failed_interface("ALGORITHM", "Invalid Input!"); continue; }
        switch (choice) {
        case 1: return Algorithm_Type::BRUTE_FORCE;
        case 2: return Algorithm_Type::GREEDY;
        case 3: return Algorithm_Type::GA;
        case 4: return Algorithm_Type::SA;
        case 5: return Algorithm_Type::ILP;
        case 6: return Algorithm_Type::EXIT;
        }
    }
}
...
string CLI::get_file(const string& folder_name) {
    ...
    if (!Helper::check_exist_file_on_folder(file)) {
        while (true) {
            ...
            cout << "Choose file: ";
            size_t choice; cin >> choice;
            if (choice < 1 || choice > file.size()) { show_failed_interface("ALGORITHM", "Invalid!"); continue; }
...
CLI::State CLI::control_run_single_algo() {
    ...
    while (at != Algorithm_Type::EXIT) {
        ...
        case(Algorithm_Type::ALGORITHM_MENU):
            ...
            if (file_name.empty()) return State::MAIN_MENU;
            at = activity_algo(); break;
        case(Algorithm_Type::BRUTE_FORCE):
            at = run_algo(file_name, Algorithms::Brute_Force, "BRUTE FORCE"); break;
        case(Algorithm_Type::GREEDY):
            at = run_algo(file_name, Algorithms::Greedy, "GREEDY"); break;
        case(Algorithm_Type::GA):
            at = run_algo(file_name, GeneticAlgorithm::run, "GENETIC ALGORITHM"); break;
        case(Algorithm_Type::SA):
            at = run_algo(file_name, SimulatedAnnealing::run, "SIMULATED ANNEALING"); break;
        case (Algorithm_Type::ILP):
            at = run_algo(file_name, Algorithms::ILP, "ILP"); break;
        case(Algorithm_Type::EXIT): break;
        }
    }
    return State::MAIN_MENU;
}
...
CLI::State CLI::control_run_all_algo() {
    ...
    if (file_name.empty()) return State::MAIN_MENU;
    Problem p(file_name);
    int n; cout << "Enter the number of time to run: "; cin >> n;
    ...
    waitting_screen(); return State::MAIN_MENU;
}
...
CLI::State CLI::activity() {
    while (true) {
        ...
        if (choose < 1 || choose > 4) { show_failed_interface("CLI", "Invalid!"); continue; }
        switch (choose) {
        case 1: return State::RUN_SINGLE_ALGO;
        case 2: return State::RUN_ALL_ALGO;
        case 3: return State::PLOT_GRAPH;
...
void CLI::main_control() {
    ...
    while (s != State::EXIT) {
        switch (s) {
        case(State::MAIN_MENU): s = activity(); break;
        case (State::RUN_SINGLE_ALGO): s = control_run_single_algo(); break;
        case(State::RUN_ALL_ALGO): s = control_run_all_algo(); break;
        case(State::PLOT_GRAPH): s = control_plot_graph(); break;
